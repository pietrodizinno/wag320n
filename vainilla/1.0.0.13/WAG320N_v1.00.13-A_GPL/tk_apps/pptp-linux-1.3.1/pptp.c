 ...
 *
 * $Id: pptp.c,v 1.1.1.1 2009-02-18 08:33:39 andy_huang Exp $
 */
...
#ifndef PATH_MAX
#define PATH_MAX 4096 /* the value comes from limits.h in kernel */
#endif
...
/*
  {
      int i;
      FILE *fp = fopen("/var/pptp.debug", "a+");
      
      if(fp){
        for(i=0; i< argc; i++){
            fprintf(fp, "[pptp] arg[%d] = %s.\n", i, argv[i]);
        }
      }
      fclose(fp);
  }
