 ...
 *
 * $Id: pptp_ctrl.c,v 1.2 2009-07-24 09:52:59 kylin_kang Exp $
 */
...
/*
void
mydbg(const char *template, ...)
{
        #define MYFILE "/tmp/john.log"
  va_list ap;
  FILE *fp=fopen(MYFILE,"a");
  if (fp==NULL) return;

  va_start (ap, template);
  vfprintf (fp, template, ap);
  va_end (ap);
  fputs("\n",fp);
  fclose(fp);
...
//  mydbg("pptp_call_open, call->call_id=%d ",call->call_id);
...
void pptp_call_destroy(PPTP_CONN *conn, PPTP_CALL *call) {
  assert(conn && conn->call); assert(call);
  assert(vector_contains(conn->call, call->call_id));
//  mydbg("pptp_call_destroy, call->call_id=%d",call->call_id);
  /* notify */
...
// mydbg("%s,valid packet",__FUNCTION__);
...
//	mydbg("%s switch to PPTP_CALL_CLEAR_NTFY",__FUNCTION__);
//	if(vector_contains(conn->call, ntoh16(packet->call_id))){
	PPTP_CALL * call;
  
    if (vector_search_peerid(conn->call, ntoh16(packet->call_id),&call)) {
//	PPTP_CALL * call;
	int err = packet->error_code;
	unsigned int legal_error_value =
	       sizeof(pptp_general_errors)/sizeof(pptp_general_errors[0]);
...
//	mydbg("%s,ready to enter ppty_call_destroy",__FUNCTION__);
	pptp_call_destroy(conn, call);
      }
