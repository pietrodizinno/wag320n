...
#ifndef INIT_SCRIPT
#define INIT_SCRIPT  "/rcS"   /* Default sysinit script. */
#endif
...
message(LOG | CONSOLE, "new_init_action:action=%x", a->action);
		...
		if ((device_open(a->terminal, O_RDWR)) < 0) {
			...
			message(LOG | CONSOLE, "Bummer, can't open %s,command(%s),action(%x)", a->terminal,a->command,a->action);
			_exit(1);
		}
...
{
	message(CONSOLE | LOG, "receice a terminal signal. and shutdown system.");
	shutdown_system();
