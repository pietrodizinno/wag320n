 ...
 *
 * $Id: shutils.c,v 1.1.1.1 2009-01-05 09:01:16 fred_fu Exp $
 */
...
_eval(char *const argv[], char *path, int timeout, int *ppid)
{
	...
	if(ppid) signal(SIGCLD, SIG_IGN);  /* now I don't have to wait()! */	  
...
safe_fread(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
	...
	do {
		clearerr(stream);
#ifdef USE_SSL
		ret += https_fread((char *)ptr + (ret * size), size, nmemb - ret, stream);
#else
