...
static int low_mem = 1;                 /* Toggle lower memory usage mode */
...
alarm_intr (int alnum)
{
  ...
  printf ("%llu... ", (unsigned long long)currently_testing);
  fflush (stdout);
}
...
establish_params (int device_num,int size)
{
  ...
  if ((0 == device_num) || ((device_num & 0xff00) == 0x0200))
	...
	unsigned long long volsize = 0;
	
	volsize=(unsigned long long)blocks*SECTORS_PER_BLOCK;
	if (volsize <= 66600)
