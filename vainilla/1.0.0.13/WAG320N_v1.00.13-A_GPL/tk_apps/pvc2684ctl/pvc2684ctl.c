...
#endif
{
  ...
  struct passwd *pw;
  // brcm
  int lencap,encap=BR2684_ENCAPS_LLC, proto_filter=0, mode=0, extif=0;
  unsigned short vlan_id=-1;
  
  bzero(&vc, sizeof(vc));

  while(1) {
    optchar = getopt(argc, argv, "Madli:v:c:g:e:rft:x:");
...
// brcm
    case 'e':
	lencap=strtol(optarg, &endptr, 10);
	if(*endptr || lencap<0){
	  printf("invalid encapsulation: %s:\n",optarg);
	  lencap=0;
	}      
      if (lencap==1)
