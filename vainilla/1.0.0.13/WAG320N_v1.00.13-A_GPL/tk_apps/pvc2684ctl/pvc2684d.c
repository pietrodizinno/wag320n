/*
	This file was original written by Marcel GAL <cell@sch.bme.hu>.
	Nikolay A. Fetisov <naf at altlinux.ru> redistributed under GPLv2. 
	Please see webpage for the information:
	http://sisyphus.ru/srpm/Sisyphus/br2684ctl/spec
	
	And please check the description of the project in following webpage:
	http://home.sch.bme.hu/~cell/br2684/USAGE.br2684
*/
...
#else
    sprintf(str, "%s%d", hdr, 0);   // force interface name to nas0 
#endif
}
...
//int create_br(int nas_idx)
{
  ...
  if((sock = socket(PF_ATMPVC, SOCK_DGRAM, ATM_AAL5)) < 0)
	...
	if (extif) {
		...
		do_error(LOG_WARNING, "atm_backend_br2684_11 = %d-----ret_11 = %d", ni.backend_num, ret);
	}
	else {
	    ni.backend_num = (g_use_bcm_atmapi == 1)
		? ATM_BACKEND_BR2684_BCM : ATM_BACKEND_BR2684;
		...
		do_error(LOG_WARNING, "atm_backend_br2684_22 = %d-----ret_22 = %d", ni.backend_num, ret);
	}
    } else {
        ni_rt.backend_num = ATM_BACKEND_RT2684;
	// brcm
        setIndexName(ni_rt.ifname, "nas", nas_idx);
        //sprintf(ni_rt.ifname, "atm_%d", nas_idx);
        ret=ioctl (sock, ATM_NEWBACKENDIF, &ni_rt);
		do_error(LOG_WARNING, "atm_backend_br2684_33 = %d-----ret_33 = %d\n", ni.backend_num, ret);
    }
    // brcm
    /*
...
//int assign_vcc(struct sockaddr_atmpvc addr, int nas_idx, int encap, int bufsize)
{
  ...
  // brcm
    ...
    // brcm
    setIndexName(be_rt.ifspec.spec.ifname, "nas", nas_idx);
