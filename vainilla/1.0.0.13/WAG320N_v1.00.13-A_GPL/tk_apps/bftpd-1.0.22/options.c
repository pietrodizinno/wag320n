...
void create_options(FILE *configfile, struct option **options, struct directory **directories)
{
    char str[256];
    struct option *opt = NULL;
    struct directory *dir = NULL;
   	fgets(str, sizeof(str), configfile);
	while (!strchr(str, '}')) {
        while ((str[0] == ' ') || (str[0] == '\t'))
            cutto(str, 1);
  		if ((str[0] != '#') && (str[0] != '\n')) {
            if ((strstr(str, "directory")) && (strchr(str, '{')) && (directories)) {
                ...
                *strchr(tmp, '"') = 0;
                ...
                create_options(configfile, &(dir->options), NULL);
       			...
       			if (opt) {
       				opt = opt->next = malloc(sizeof(struct option));
       			} else {
       				*options = opt = malloc(sizeof(struct option));
       			}
...
void config_init()
{
	...
	while (fgets(str, sizeof(str), configfile)) {
		if (strchr(str, '{')) {
            while ((str[0] == ' ') || (str[0] == '\t'))
                cutto(str, 1);
    		if (str[0] == '#')
...
char *getoption(struct option *opt, char *name)
{
	do {
