...
//Added by Shearer to support Thumbnai on web		
...
//Added by Shearer to support Thumbnai on web		
		...
		DEBUG(5,("close_file: file %s. Delete on close was set - deleting file.\n",
			fsp->fsp_name));
		if(IsPicture(fsp->fsp_name)){
	#define	THUMBNAIL_PREFIX	"Thumbnail_"
	#define	THUMB_FOLDER		".Thumb_DB"		
			pThumb=malloc(strlen(fsp->fsp_name)+strlen(THUMBNAIL_PREFIX)+strlen(THUMB_FOLDER)+4);
			if(pThumb){
#undef sprintf				
				memset(pThumb, 0, strlen(fsp->fsp_name)+strlen(THUMBNAIL_PREFIX)+strlen(THUMB_FOLDER)+4);	
				sprintf(pThumb, THUMB_FOLDER"/"THUMBNAIL_PREFIX"%s", fsp->fsp_name);
