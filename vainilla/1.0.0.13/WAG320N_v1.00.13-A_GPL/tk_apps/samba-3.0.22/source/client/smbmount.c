...
static BOOL smbfs_has_lfs = True; //False;
 ...
 again:
	...
	if (!cli_session_setup(c, username, 
		...
		if (password[0] || !username[0] ||
			...
			fp=fopen("/tmp/smbmount.err","wt");
			if(fp){
				fprintf(fp,"%d",SVAL(c->inbuf,smb_err));
				fclose(fp);	
			}					
...
static void init_mount(void)
{
	...
	*/
	...
	if (sys_fork() == 0) {
		sleep(4);
		execv("/usr/bin/smbmnt",args);
		fprintf(stderr,	"smbfs/init_mount: execv of  failed. Error was %s.",strerror(errno));
#if 0
		char *smbmnt_path;
