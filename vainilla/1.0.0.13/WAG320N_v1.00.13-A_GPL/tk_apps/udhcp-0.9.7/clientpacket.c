...
static void init_packet(struct dhcpMessage *packet, char type)
{
	...
	// mark domain name --iceman
	//if (client_config.domainname) add_option_string(packet->options, client_config.domainname);
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
	...
	u_int16_t check,check_sum_ip,check_sum_all;
	...
	packet.ip.check = 0;
	check_sum_ip=checksum(&(packet.ip), sizeof(packet.ip));
	if (check != check_sum_ip) {
	...
	if(bytes%2!=0)
		check_sum_all=checksum(&packet, bytes+1);
	else
		check_sum_all=checksum(&packet, bytes);
	if (check && check != check_sum_all) {
