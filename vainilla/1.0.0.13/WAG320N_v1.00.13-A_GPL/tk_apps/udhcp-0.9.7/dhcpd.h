...
struct reserve_ip
{
  char res_devicename[32];
    unsigned char res_ip[16];
    unsigned char res_mac[18];
};
struct option_set {
	unsigned char *data;
...
struct server_config_t {
	...
	u_int32_t vid; 	/* virtual server id*/
	u_int32_t enable; 		/*server enable*/
	char *lease_file;
	char *pidfile;
...
extern struct server_config_t server_config[MAX_INTERFACES][MAX_SERVERS_PER_IF];
extern struct dhcpOfferedAddr *leases[MAX_INTERFACES][MAX_SERVERS_PER_IF];
extern int no_of_ifaces;
extern int no_of_servers[MAX_INTERFACES];
#endif
