/* $Id: iptcrdr.c,v 1.2 2009-04-30 06:14:50 shearer_lu Exp $ */
/* MiniUPnP project
...
add_redirect_rule2(const char * ifname,const char * eaddr, unsigned short eport, 
				   ...
				   const char * desc)
{
	int r = addnatrule(proto,eaddr, eport, iaddr, iport);
	if(r >= 0)
           ...
           const char * iaddr, unsigned short iport)
{
	...
	if(eaddr!= NULL && eaddr[0]!= '\0')
	{
		e->ip.src.s_addr = inet_addr(eaddr);
		e->ip.smsk.s_addr =  inet_addr("255.255.255.255");
	}
	if((wanip = getWanIPAddress())!= NULL)
	{
		e->ip.dst.s_addr = inet_addr(wanip);
