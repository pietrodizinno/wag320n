...
unsigned int host_uniq_index=0x00000020;
unsigned int host_uniq_padi=0x00000020;
...
void copy_tag(struct pppoe_packet *dest, struct pppoe_tag *pt)
{
	...
	if(tagid == TAG_HOST_UNIQ)
        memcpy((char *)end_tag+8,&host_uniq_index,sizeof(host_uniq_index));
	dest->hdr->length = htons(ntohs(dest->hdr->length) + tag_len);

    }else{
	memcpy(next_tag(dest->hdr),pt, tag_len);
    if(tagid==TAG_HOST_UNIQ)
        memcpy((char *)next_tag(dest->hdr)+8,&host_uniq_index,sizeof(host_uniq_index));
...
{
    char buf[MAX_PAYLOAD + sizeof(struct pppoe_hdr)] = {0};
	...
	data_len += (sizeof(struct pppoe_tag) +
		     sizeof(struct session *))+4;
	tag = next_tag(ph);
    tag->tag_type = PTT_HOST_UNIQ;
	tag->tag_len = htons(sizeof(struct session *)+4);
	memcpy(tag->tag_data,
	       &ses,
	       sizeof(struct session *));
    memcpy(tag->tag_data+4, &host_uniq_index, 4);
//    memcpy(tag->tag_data+4, "aaaa", 4);
	add_tag(ph, tag);
    }
...
#define CHECK_TAG(name, val)					\
    ...
    if( hu_val != ses ){
    	...
    	poe_info(ses,"HOST_UNIQ mismatch: %08x %d\n",(int)hu_val,getpid());
    	return -1;
    }
...
int session_disconnect(struct session *ses){
    ...
    if(disc_sock<0)
    {
        ...
        if( bind( disc_sock, (struct sockaddr*)&ses->local, sizeof(struct sockaddr_ll)) < 0 )
        {
            close(disc_sock);
            disc_sock = -1;
            return 0;
...
int session_connect(struct session *ses)
{
    ...
    while(ses->retransmits < ses->retries || ses->retries==-1 ) {
    	...
    	if( ret == 0 ){
			...
			if(!pppoe_relay && times==2) {
				/* for no timeout process, just only repeat twice send_disc     */
    	    ...
    	    if( ses->timeout ){
                ...
                {
                    if(ses->retransmits <= 4) 
                        poe_info(ses,"Sending PADI %d...\n",ses->retransmits);
                    else
                        break;
                }
    		send_disc(ses,p_out);
    ...
    /* in Forthnet, if send PADI and no response, have to reset adsl */
    if(ses->retransmits >= 5){
        if(p_out->hdr->code==PADI_CODE){
            poe_info(ses,"Restart adsl...\n");
            system("/usr/sbin/rc adsl start");
        }
    }
