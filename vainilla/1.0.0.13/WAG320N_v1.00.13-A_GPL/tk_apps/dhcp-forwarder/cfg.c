// $Id: cfg.c,v 1.1.1.1 2009-01-05 09:01:18 fred_fu Exp $    --*- c++ -*--
...
#define PACKAGE_STRING      "dhcp-forwarder 0.4"
#define CFG_FILENAME        "/etc/dhcp-fwd.conf"
#define CFG_PIDFILE		    "/var/run/dhcp-fwd.pid"
#define CFG_LOGFILE		    "/var/log/dhcp-fwd.log"
#define CFG_CHROOT		    "/var/lib/dhcp-fwd"
#define CFG_LOGLEVEL        1
#define CFG_USER            0
#define CFG_GROUP           0
...
debug("the inface->name is %s, the fd is %d, \nhas_client is %d, has_servers is %d",
    iface->name, fd->fd,iface->has_clients,iface->has_servers);
...
debug("the len of the ifs is %d", ifs->len);
...
debug("the ifinfo->name is %s", ifinfo->name);
	     ...
	     /\ maxRead(cfg->pidfile_name)>=0@*/
{
    ...
    strcpy(cfg->chroot_path, CFG_CHROOT);
    strcpy(cfg->logfile_name, CFG_LOGFILE);
    cfg->loglevel = CFG_LOGLEVEL;

    strcpy(cfg->pidfile_name, CFG_PIDFILE);

    parse(filename, cfg);
    debug("the file is %s",filename);
    /*@-boundsread@*/
		 ...
		 struct FdInfoList *		fds)
{
    ...
    printf("[%s:%s() %d] cfg.pidfile_name is <%s>\n", __FILE__, __FUNCTION__, __LINE__, cfg.pidfile_name);
