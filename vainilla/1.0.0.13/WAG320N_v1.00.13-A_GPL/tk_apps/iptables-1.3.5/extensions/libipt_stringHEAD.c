...
{
	printf(
"STRING_HEAD match v%s options:\n"
"--stringHEAD [!] string         Match a string in the beginning of each line in HTTP head\n",
IPTABLES_VERSION);
...
{	
	...
	if (slen == 0) {
		exit_error(PARAMETER_PROBLEM,
			"STRING_HEAD must contain at least one char");
	}
...
final_check(unsigned int flags)
{
	if (!flags)
		exit_error(PARAMETER_PROBLEM,
			   "STRING_HEAD match: You must specify `--string'");
}
...
static
struct iptables_match stringHEAD
= {
    .name			= "stringHEAD",
