...
/*
 * This fdisk is very very interesting!! If you want to change the disk number, 
 * you need change this value.              Argon Cheng 2004-05-19
 */
...
static void
change_sysid(int partition) {
	char *temp;
//	int i = get_partition(0, partitions), sys, origsys;
	int i = partition, sys, origsys;
...
static void
add_partition(int n, int sys,unsigned long *partition_start,unsigned long *partition_stop) {
...
//					 0, mesg);
	...
	if ((cround(start) == cround(limit))/* || (n==3) */) {
		stop = limit;
	} else {
...
static void
new_partition(int partition,unsigned long *partition_start,unsigned long *partition_stop, int type) {
	...
	if (!free_primary) {
		...
		if(type==81)
			add_partition(partition,
				0xfd,partition_start,partition_stop);
		else if(type==80)
			add_partition(partition,
				0x8e,partition_start,partition_stop);		
		else
			add_partition(partition,
				LINUX_NATIVE,partition_start,partition_stop);
			return;
	}
}
...
#define FDISKFILE "/var/fdisk"
int
main(int argc, char **argv) {
	int j, c;
	int optl = 0, opts = 0, optd = 0;
	 ...
	 */
	while ((c = getopt(argc, argv, "b:ldsuvV")) != EOF) {
		switch (c) {
		case 'b':
...
#endif
	{
		FILE *fp;
		char buf[64];
		unsigned long cylinders,stop;
		unsigned long start;
		int i=0,type;

		fp=fopen(FDISKFILE,"rt");
		if(!fp)
			return -1;
		while(fgets(buf,sizeof(buf),fp)){
			if(sscanf(buf,"%lu,%ld,%x\n",&start,&cylinders,&type) != 3){
        		fclose(fp);
        		return -1;
			}
    		stop=start+cylinders;
    		//printf("start: %lu, stop: %lu\n", start, stop);
    		add_partition(i,type,&start,&stop);
    		i++;
		}
    	fclose(fp);
    	write_table();
