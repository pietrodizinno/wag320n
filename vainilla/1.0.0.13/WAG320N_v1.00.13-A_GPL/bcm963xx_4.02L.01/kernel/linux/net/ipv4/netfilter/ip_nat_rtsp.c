                 ...
                 struct sk_buff** pskb, uint tranoff, uint tranlen)
{
    ...
    extaddrlen = extip ? sprintf(szextaddr, "%u.%u.%u.%u", NIPQUAD(extip))
                       : sprintf(szextaddr, "%u.%u.%u.%u", NIPQUAD(exp->tuple.dst.ip));
#else
    extaddrlen = extip ? sprintf(szextaddr, "%u.%u.%u.%u", NIPQUAD(extip))
                ...
                struct ip_conntrack* ct, struct ip_nat_info* info)
{
    ...
    DEBUGP("newsrcip=%u.%u.%u.%u, newdstip=%u.%u.%u.%u, newip=%u.%u.%u.%u CLIENT PORT %hu\n",
     ...
     struct sk_buff** pskb)
{
    ...
    DEBUGP("SRC IP %u.%u.%u.%u DST IP %u.%u.%u.%u PORTS %hu-%hu\n",
           NIPQUAD(iph->saddr), NIPQUAD(iph->daddr), tcph->source,
           tcph->dest);
