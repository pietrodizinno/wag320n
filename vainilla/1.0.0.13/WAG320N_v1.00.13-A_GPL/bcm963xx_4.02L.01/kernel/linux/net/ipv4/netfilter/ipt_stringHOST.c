...
#include <linux/netfilter_ipv4/ipt_stringHOST.h>
...
struct string_per_cpu *bm_string_data_host=NULL;
...
char *search_sublinear (char *needle, char *haystack, int needle_len, int haystack_len) 
{
	...
	shift=bm_string_data_host[smp_processor_id()].shift;
	skip=bm_string_data_host[smp_processor_id()].skip;
	len=bm_string_data_host[smp_processor_id()].len;
...
char *search_host_linear (char *needle, char *haystack, int needle_len, int haystack_len) 
{
      ...
      int *hotdrop)
{
	...
	proc_ipt_search search=search_host_linear;
	...
	if ( (hlen>IPT_STRING_HAYSTACK_THRESH) &&
		...
		if ( hlen < BM_MAX_HLEN ) {
			search=search_host_linear;//search=search_sublinear;
		}else{
			if (net_ratelimit());
		}
	}
...
static void string_freeup_data(void)
{
	...
	if ( bm_string_data_host ) {
		for(c=0; c<smp_num_cpus; c++) {
			if ( bm_string_data_host[c].shift ) kfree(bm_string_data_host[c].shift);
			if ( bm_string_data_host[c].skip ) kfree(bm_string_data_host[c].skip);
			if ( bm_string_data_host[c].len ) kfree(bm_string_data_host[c].len);
		}
		kfree(bm_string_data_host);
	}
}

static struct ipt_match stringHOST_match =
{ 
    .list = { NULL, NULL },
    .name = "stringHOST",
    .match = &match,
...
static int __init init(void)
{
	...
	if ( !(bm_string_data_host=kmalloc(tlen,GFP_KERNEL)) ) {
		return 0;
	}
	
	memset(bm_string_data_host, 0, tlen);
	
	/* allocate our skip/shift tables */
	for(c=0; c<smp_num_cpus; c++) {
		if ( !(bm_string_data_host[c].shift=kmalloc(alen, GFP_KERNEL)) )
			goto alloc_fail;
		if ( !(bm_string_data_host[c].skip=kmalloc(alen, GFP_KERNEL)) )
			goto alloc_fail;
		if ( !(bm_string_data_host[c].len=kmalloc(alen, GFP_KERNEL)) )
			goto alloc_fail;
	}
	
	return xt_register_match(&stringHOST_match);
...
{
	xt_unregister_match(&stringHOST_match);
	string_freeup_data();
}
