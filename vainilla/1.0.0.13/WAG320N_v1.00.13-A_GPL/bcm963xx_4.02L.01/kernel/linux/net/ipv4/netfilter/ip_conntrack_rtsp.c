                        ...
                        struct ip_conntrack *ct)
{
    ...
    for (i = 0; i < MAX_PORT_MAPS; i++) {
        ...
        // Check whether the current using ports is the same as the passed port, if it is,
        // use port+2 to instead.
        for(j = 0; j < MAX_PORT_MAPS; j++)
        {
            if(rtsp_data_ports[j].in_use)
            {
                if(prtspexp->loport == rtsp_data_ports[j].nat_udp_lo)
                {
                    prtspexp->loport += 2;
                    continue;	
                }	
        ...
        // Now our prtspexep->loport is not conflict with others
        rtsp_data_ports[i].nat_udp_lo = prtspexp->loport;
        prtspexp->hiport = prtspexp->loport + 1;
        rtsp_data_ports[i].nat_udp_hi = prtspexp->hiport;
        rtsp_data_ports[i].in_use          = 1;
...
static void
#if defined(CONFIG_SERCOMM_CODE)
ip_conntrack_rtsp_proc_teardown(struct iphdr *iph, struct ip_conntrack* ct)
#else
...
#if defined(CONFIG_SERCOMM_CODE)
    // Start change by Bernard to test rtsp teardown issue
...
#if defined(CONFIG_SERCOMM_CODE)
    // When teardown is received, reset g_tr_port to 0;
    g_tr_port = 0;
    
    // Start change by Bernad to test rtsp teardown issue.
	  for (cur_item = ct->sibling_list.next; cur_item != &ct->sibling_list; cur_item = next) 
	  {
		    next = cur_item->next;
                ...
                struct ip_conntrack* ct, enum ip_conntrack_info ctinfo)
{
    ...
    while (dataoff < datalen)
    {
        ...
        {
#if defined(CONFIG_SERCOMM_CODE)
            // pass ct to the function in order to remove related expected connections.
            ip_conntrack_rtsp_proc_teardown(iph, ct);   /* TEARDOWN message */
#else
       ...
       /*
        ...
        while (port <= exp.help.exp_rtsp_info.hiport) {
            ...
            if (ret == 2) {
#if defined(CONFIG_SERCOMM_CODE)
                    new_exp->tuple.dst.u.udp.port = htons(port + g_tr_port);
                    //g_tr_port++;
#else
...
#if defined(CONFIG_SERCOMM_CODE)
        // For: if one client send more than two connections.
        g_tr_port += 2;
#endif        
        UNLOCK_BH(&ip_rtsp_lock);
    }
