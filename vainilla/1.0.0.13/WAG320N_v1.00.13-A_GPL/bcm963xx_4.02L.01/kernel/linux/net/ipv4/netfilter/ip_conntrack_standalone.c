...
static int ct_seq_show_my(struct seq_file *s, void *v)
{
	const struct ip_conntrack_tuple_hash *hash = v;
	const struct ip_conntrack *conntrack = tuplehash_to_ctrack(hash);
	...
	switch ( protonum )
	{
		...
		default:
			seq_printf(s, "protocol=%d  ", protonum );
			break;
	}

	// timeout
	if(seq_printf(s, "T=%lu  ", 
		timer_pending(&conntrack->timeout)? (long)(conntrack->timeout.expires - jiffies)/HZ : 0 ) !=0)
		return -ENOSPC;

	// ip
	seq_printf(s, "%u.%u.%u.%u: %hu  => %u.%u.%u.%u: %hu  ",
...
*/
	// TCP state
	proto->print_conntrack(s, conntrack);
...
static struct seq_operations ct_seq_ops_my = {
	.start = ct_seq_start,
	.next  = ct_seq_next,
	.stop  = ct_seq_stop,
	.show  = ct_seq_show_my
...
static int ct_open_my(struct inode *inode, struct file *file)
{
	struct seq_file *seq;
	struct ct_iter_state *st;
	int ret;
	...
	if (st == NULL)
		return -ENOMEM;
	ret = seq_open(file, &ct_seq_ops_my);
	if (ret)
		goto out_free;
	seq          = file->private_data;
	seq->private = st;
...
static const struct file_operations ct_file_ops_my = {
	.owner   = THIS_MODULE,
	.open    = ct_open_my,
	.read    = seq_read,
	.llseek  = seq_lseek,
...
//	proc_1 = proc_net_create("ct_list_1",0,list_conntracks_1);
	proc_1 = proc_net_fops_create("ct_list_1", 0440, &ct_file_ops_my);
