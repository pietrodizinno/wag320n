...
{
	const struct ip_conntrack *ctrack = tuplehash_to_ctrack(i);
	return ((ctrack->reserve.mask!=IPS_RESERVE) && (!(test_bit(IPS_ASSURED_BIT, &ctrack->status))));
}
#endif
...
static int early_drop_my(struct list_head *chain)
{
        ...
        int dropped = 0;
	const struct ip_conntrack *ctrack = tuplehash_to_ctrack(h);

        READ_LOCK(&ip_conntrack_lock);
        h = LIST_FIND_B(chain, unreplied_my, struct ip_conntrack_tuple_hash *);
        if (h)
                atomic_inc(&ctrack->ct_general.use);
        ...
        if (del_timer(&ctrack->timeout)) {
                death_by_timeout((unsigned long)ctrack);
                dropped = 1;
        }
...
{
	const struct ip_conntrack *ctrack = tuplehash_to_ctrack(i);
        return ctrack->reserve.mask!=IPS_RESERVE;
}
...
/* this #endieis guaranteed to always return a valid protocol helper, since
...
#endif
#if defined(CONFIG_IP_NF_NAT_NEEDED)	// kylin
...
#if defined(CONFIG_SERCOMM_CODE)
///////////////////Crimson Added for Multi PPTP Passthrough 2005-12///////////////////////////
