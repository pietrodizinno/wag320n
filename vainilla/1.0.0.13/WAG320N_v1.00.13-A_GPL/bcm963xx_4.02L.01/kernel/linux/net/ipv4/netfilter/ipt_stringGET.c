...
#include <linux/netfilter_ipv4/ipt_stringGET.h>
...
struct string_per_cpu *bm_string_data_get=NULL;
...
char *search_sublinear (char *needle, char *haystack, int needle_len, int haystack_len) 
{
	...
	shift=bm_string_data_get[smp_processor_id()].shift;
	skip=bm_string_data_get[smp_processor_id()].skip;
	len=bm_string_data_get[smp_processor_id()].len;
...
/* Search string in line which include "GET" - Writer by Jeff Sun - Mar.2.2005 - */
char *search_get_linear (char *needle, char *haystack, int needle_len, int haystack_len) 
{
      ...
      int *hotdrop)
{
	...
	proc_ipt_search search=search_get_linear;
	...
	if ( (hlen>IPT_STRING_HAYSTACK_THRESH) &&
		...
		if ( hlen < BM_MAX_HLEN ) {
			search=search_get_linear;//search=search_sublinear;
		}else{
			if (net_ratelimit());
		}
	}
...
static struct ipt_match stringGET_match =
{ 
    .list = { NULL, NULL },
    .name = "stringGET",
    .match = &match,
...
static int __init init(void)
{
	...
	return xt_register_match(&stringGET_match);
...
{
	xt_unregister_match(&stringGET_match);
	string_freeup_data();
}
