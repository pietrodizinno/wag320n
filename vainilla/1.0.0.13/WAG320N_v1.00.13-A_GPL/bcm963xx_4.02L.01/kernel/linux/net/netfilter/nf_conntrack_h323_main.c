...
static int safe_calling_mode __read_mostly = 0;
...
/* Hooks for NAT */
int (*set_addr_bf_hook)(struct sk_buff **pskb,
...
/****************************************************************************/
int have_direct_route(union nf_conntrack_address *src,
	...
	case AF_INET: {
		...
		if (ip_route_output_key(&rt1, &fl1) == 0) {
			if (ip_route_output_key(&rt2, &fl2) == 0) {
				if (((rt1->u.dst.flags & DST_HOST) ||
				    (rt2->u.dst.flags & DST_HOST) ||
			 ...
			 unsigned char **data, int *datalen, int *dataoff)
{
	...
	if (*data == NULL) {	/* first TPKT */
			...
			if (info->tpkt_len[dir] > 0) {
				...
				if (info->tpkt_len[dir] <= tcpdatalen) {
					...
					*dataoff = 0;
					...
					/* BRCM: there will be another part
		...
		/*BRCM: Do not clear that, just break when it returns from
		 * this procedure*/
	...
	if (tpktlen > tcpdatalen) {
		if (tcpdatalen == 4) {	/* Separate TPKT header */
			...
			/* Some kind of Clients send large Q.931 pkt which is
			 * divided into PKTS by tcp layer when it is
			 * transferred.*/
			   ...
			   H245_TransportAddress *taddr)
{
	...
	if (!get_h245_addr(ct, *data, taddr, &addr, &port) || port == 0)
		return 0;
	...
	if (memcmp(&addr, &ct->tuplehash[dir].tuple.src.u3, sizeof(addr))) {
		...
		return 0;
#else
		DEBUGP("nf_ct_h323: H.323 signal proxy detected\n");
		/* The derived connections no need to go through firewall */
		if (have_direct_route(&ct->tuplehash[!dir].tuple.src.u3,
				      &addr,
				      ct->tuplehash[!dir].tuple.src.l3num)) {
			DEBUGP("nf_ct_h323: proxied EP has direct route to "
			       "peer\n");
			return 0;
		}
#endif
	...
	nf_conntrack_expect_init(rtp_exp, ct->tuplehash[!dir].tuple.src.l3num,
				 ...
				 &addr, IPPROTO_UDP, NULL, &rtp_port);
	...
	nf_conntrack_expect_init(rtcp_exp, ct->tuplehash[!dir].tuple.src.l3num,
				 safe_calling_mode?
				 &ct->tuplehash[!dir].tuple.src.u3 : NULL,
				 &addr, IPPROTO_UDP, NULL, &rtcp_port);
		     ...
		     struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	int dataoff;
	int ret;
	typeof(set_addr_bf_hook) set_addr_bf;
	struct nf_ct_h323_master *info = &nfct_help(ct)->help.ct_h323_info;
	int dir = CTINFO2DIR(ctinfo);
	...
	while (get_tpkt_data(pskb, protoff, ct, ctinfo,
		...
		if (info->div_type[dir] != DIVTYPE_Q931) {//BRCM
			/* Decode H.245 signal */
			ret = DecodeMultimediaSystemControlMessage(data,
								   datalen,
								   &mscm);
			if (ret < 0) {
				if (net_ratelimit())
					printk("nf_ct_h245: decoding error: "
					       "%s\n",
			...
			if (process_h245(pskb, ct, ctinfo, &data, dataoff,
					 &mscm) < 0)
				goto drop;
		} else {
	    		if ((set_addr_bf = rcu_dereference(set_addr_bf_hook))
			    && ct->status & IPS_NAT_MASK) {
				//BRCM: brute force
				if (set_addr_bf(pskb, &data, datalen, dataoff)
				    < 0)
					goto drop;
			} else {
				break;
				 ...
				 TransportAddress *taddr)
{
	...
	if (have_direct_route(&addr, &ct->tuplehash[!dir].tuple.src.u3,
			      ct->tuplehash[!dir].tuple.src.l3num)) {
		     ...
		     struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (get_tpkt_data(pskb, protoff, ct, ctinfo,
		...
		if (info->div_type[dir] != DIVTYPE_Q931) {//BRCM
			...
			if (ret < 0) {
				if (net_ratelimit())
					printk("nf_ct_q931: decoding error: "
					       "%s\n",
					       ret == H323_ERROR_BOUND ?
					       "out of bound" :
					       "out of range");
			...
			if (process_q931(pskb, ct, ctinfo, &data, dataoff,
					 &q931) < 0)
				goto drop;
		} else {
	    		if ((set_addr_bf = rcu_dereference(set_addr_bf_hook))
		       ...
		       TransportAddress *taddr, int count)
{
	...
	if ((exp = nf_conntrack_expect_alloc(ct)) == NULL)
		return -1;
	nf_conntrack_expect_init(exp, ct->tuplehash[!dir].tuple.src.l3num,
				 safe_calling_mode ? /* only accept calls from GK? */
				 &ct->tuplehash[!dir].tuple.src.u3 : NULL,
				 &addr, IPPROTO_TCP, NULL, &port);
	exp->helper = nf_conntrack_helper_q931;
...
EXPORT_SYMBOL_GPL(set_addr_bf_hook);
