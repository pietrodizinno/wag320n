...
static unsigned int nf_ct_tcp_timeout_established __read_mostly =   7 MINS; //5 DAYS;
static unsigned int nf_ct_tcp_timeout_fin_wait __read_mostly =      2 MINS;
static unsigned int nf_ct_tcp_timeout_close_wait __read_mostly =   30 SECS; //60 SECS;
static unsigned int nf_ct_tcp_timeout_last_ack __read_mostly =     30 SECS;
static unsigned int nf_ct_tcp_timeout_time_wait __read_mostly =     9 SECS; // 2 MINS;
			 ...
			 int pf)
{
	...
	DEBUGR("tcp_in_window: START\n");
	DEBUGR("tcp_in_window: src=%u.%u.%u.%u:%hu dst=%u.%u.%u.%u:%hu "
	       "seq=%u ack=%u sack=%u win=%u end=%u\n",
		NIPQUAD(skb->nh.iph.saddr), ntohs(tcph->source),
		NIPQUAD(skb->nh.iph.daddr), ntohs(tcph->dest),
		seq, ack, sack, win, end);
	DEBUGR("tcp_in_window: sender end=%u maxend=%u maxwin=%u scale=%i "
	...
	DEBUGR("tcp_in_window: I=%i II=%i III=%i IV=%i\n",
	...
	DEBUGR("tcp_in_window: res=%i sender end=%u maxend=%u maxwin=%u "
