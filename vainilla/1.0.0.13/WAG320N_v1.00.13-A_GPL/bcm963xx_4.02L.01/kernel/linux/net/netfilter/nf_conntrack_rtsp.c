...
static DEFINE_SPINLOCK(rtsp_buffer_lock);
	...
	struct nf_conntrack_tuple g_tuples[2];
	int g_tuples_count = 0;
	EXPORT_SYMBOL(g_tuples);
	EXPORT_SYMBOL(g_tuples_count);
#endif
...
static void rtsp_destroy_siblings(struct nf_conn *ct)
{
	struct nf_conn_help *help = nfct_help(ct);
	struct nf_conntrack_tuple t;
	struct nf_conntrack_tuple_hash *h; 
	struct nf_conntrack_expect *exp;
	struct nf_conn *sibling, *del_siblings[2], *del_sibling;
	int i,n;
	...
	for (i = 0; i < nf_conntrack_htable_size; i++) {
		...
		list_for_each_entry(h, &nf_conntrack_hash[i], list) {
			...
			sibling = nf_ct_tuplehash_to_ctrack(h);
			if (sibling->master == ct && sibling != ct) {
				del_siblings[n++] = sibling;
			}
		}

		for(n=0; n<2; n++) {
			del_sibling = del_siblings[n];
			if (del_sibling)
			{
				DEBUGP("setting timeout of conntrack %p to 0\n", del_sibling);
				memcpy(&t, &del_sibling->tuplehash[IP_CT_DIR_ORIGINAL].tuple, sizeof(t));
				NF_CT_DUMP_TUPLE(&t);
				del_sibling->proto.gre.timeout	  = 0;
				del_sibling->proto.gre.stream_timeout = 0;
				if (del_timer(&del_sibling->timeout)){
					del_sibling->timeout.function((unsigned long)del_sibling);
				}
				nf_ct_put(del_sibling);
				del_sibling = NULL;
			}
		}
	}
                ...
                struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		if (strncmp(pdata+cmdoff, "TEARDOWN ", 9) == 0) {
			DEBUGP("kdebug TEARDOWN, rtsp_destroy_siblings\n");
			rtsp_destroy_siblings(ct);
...
#if defined(CONFIG_SERCOMM_CODE)
			//exp->mask.dst.u.udp.port  = 0xfffe;	/* kylin: expect_related the 2 ports seperately. */
#else
