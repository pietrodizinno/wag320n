...
static void ftp_destroy_siblings(struct nf_conn *ct)
{
...
static int find_nl_seq(u32 seq, const struct nf_ct_ftp_master *info, int dir)
{
	...
	DEBUGP("<kdebug> find_nl_seq k: seq %x\n", seq);
	for (i = 0; i < info->seq_aft_nl_num[dir]; i++) {
		DEBUGP("<kdebug> find_nl_seq: seq_aft_nl_num[%d]: %x\n",i, info->seq_aft_nl[dir][i]);
		if (info->seq_aft_nl[dir][i] == seq){
			DEBUGP("<kdebug> find_nl_seq: seq_aft_nl_num[%d]: %x; EQUAL\n",i, info->seq_aft_nl[dir][i]);
			return 1;
		}
	}
	return 0;
...
#if defined(CONFIG_SERCOMM_CODE)
    /* after received the FIN/ACK of ftp control session, closing the data session */
	if (th->fin && th->ack && (th->source == FTP_PORT || th->dest == FTP_PORT)) {
		DEBUGP("ftp: FIN/ACK received.\n");
		
		/* unexpect related. */
		ftp_destroy_siblings(ct);
	}
#endif
...
#if defined(CONFIG_SERCOMM_CODE)
	else if ( wrong_seq_pos == 1 && search[dir][i].ftptype != NF_CT_FTP_PORT) {
			DEBUGP("nf_conntrack_ftp, not PORT command retransmission.\n"); /* cdrouter PORT bug */
			goto out_update_nl;
	}
#endif
