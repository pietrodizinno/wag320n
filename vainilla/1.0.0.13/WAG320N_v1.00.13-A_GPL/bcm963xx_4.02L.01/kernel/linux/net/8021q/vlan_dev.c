...
{
#if defined(CONFIG_MIPS_BRCM)
   /* egress priority map based on skb->vtag instead of skb->priority */
	struct vlan_priority_tci_mapping *mp =
		VLAN_DEV_INFO(dev)->egress_priority_map[(skb->vtag & 0xF)];

    	while (mp) {
		if (mp->priority == (skb->vtag & 0xF)) {
...
int vlan_dev_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    * rule, we want to restore its original vlan tag from skb->vtag_save before
    * doing double tagging.
    * if bit 12 of skb->vtag_save is set, it means this vlan frame had been remarked.
    * 
    */
	if ((veth->h_vlan_proto == __constant_htons(ETH_P_8021Q)) &&
       (skb->vtag_save & 0x1000)) {
