 ...
 **/
static void print_inquiry(unsigned char *inq_result, char *vendor_str)
{
	int i,j=0;
		...
		int *bflags, int async)
{
	...
	print_inquiry(inq_result, vendor_str);
	...
	if(p)
		*p=0;
	printk("vendor_str=%s, length=%d\n",vendor_str, strlen(vendor_str));
	if(strcmp(vendor_str, "WD") && !sdev->removable){
		unsigned long  capacity=0;
#define	MIN_HDD_SIZE	(10*1024)
		capacity=sd_read_capacity_only(sdev);
		printk("capacity: %lu MB\n", capacity);
	    if(capacity && capacity < MIN_HDD_SIZE)
			sdev->removable = 1;
	}
#endif
