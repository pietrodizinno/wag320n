...
#if defined(CONFIG_SERCOMM_CODE)
unsigned long sd_read_capacity_only(struct scsi_disk *sdkp)
{
	unsigned char cmd[16];
	int the_result, retries;
	int sector_size = 0;
...
repeat:
	...
	if (!longrc) {
		...
		capacity = 1 + (((sector_t)buffer[0] << 24) |
			(buffer[1] << 16) |
			(buffer[2] << 8) |
			buffer[3]);			
	} else {
		capacity = 1 + (((u64)buffer[0] << 56) |
			((u64)buffer[1] << 48) |
			((u64)buffer[2] << 40) |
			((u64)buffer[3] << 32) |
			((sector_t)buffer[4] << 24) |
	 ...
	 * and err on the side of lowering the capacity. */
		...
		if (sdp->guess_capacity)
			if (capacity & 0x01) /* odd sizes are odd */
				--capacity;
	}
got_data:
	if (sector_size == 0) {
	...
	*/
	...
	queue = sdp->request_queue;
	sz = (capacity/2) * (sector_size/256);
	mb = sz;
	blk_queue_hardsect_size(queue, hard_sector);	
	/* avoid 64-bit division on 32-bit platforms */
	sector_div(sz, 625);
	mb -= sz - 974;
	sector_div(mb, 1950);
	printk(KERN_NOTICE "SCSI device: "
		       "%llu %d-byte hdwr sectors (%llu MB)\n",
		      (unsigned long long)capacity,
		       hard_sector, (unsigned long long)mb);	
	kfree(buffer);
	return (unsigned long)mb;
}
#endif
...
#if defined(CONFIG_SERCOMM_CODE)
	gd->major = 8;
	gd->first_minor = 0 + 16*index;
	gd->minors = 16;
	gd->fops = &sd_fops;
	sprintf(gd->disk_name, "sd%c", 'a' + index);	
	strcpy(sdp->node, gd->disk_name);
#else
...
#if defined(CONFIG_SERCOMM_CODE)
	if(sdp->removable){
		sprintf(proc_name,"flash_%s",gd->disk_name);
	}
	else	{
		sprintf(proc_name,"hdd_%s",gd->disk_name);
	}
	create_proc_entry(proc_name, 0, NULL);
#endif
	return 0;
...
#if defined(CONFIG_SERCOMM_CODE)
	//Remove proc flag such as flash_sda, hdd_sdb

	if(sdkp->device->removable){
		sprintf(proc_name,"flash_%s",sdkp->disk->disk_name);
	}
	else{
		sprintf(proc_name,"hdd_%s",sdkp->disk->disk_name);
	}
