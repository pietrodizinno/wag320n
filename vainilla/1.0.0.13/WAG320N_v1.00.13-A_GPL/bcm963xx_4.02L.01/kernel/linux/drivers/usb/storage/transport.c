 ...
 *
 * $Id: transport.c,v 1.3 2009-06-11 07:49:48 shearer_lu Exp $
 *
...
unsigned long usb_turn_on_time=0;
#define ON_LED_INTERVAL 		25
#define GPIO_USB 	    		(1<<2)
int usb_blink=0; //0-Disabled, 1-Ready

static struct timer_list usb_led_timer;    /* ide led switch */

void led_off(unsigned long gpio,int logic);
void led_on(unsigned long gpio,int logic);

void turn_on_led_usb(unsigned long ptr)
{
	led_on(GPIO_USB,-1);
	return;
}

void turn_off_led_usb(unsigned long ptr)
{
	led_off(GPIO_USB,-1);
	return;
}

void setup_usb_led_timer(void)
{
	init_timer(&usb_led_timer);
	usb_led_timer.function = turn_on_led_usb;
	usb_led_timer.expires = jiffies + ON_LED_INTERVAL;
...
#if defined(CONFIG_SERCOMM_CODE)
	/*blink USB LED */
	if(usb_blink && (jiffies-usb_turn_on_time)>=JIFFIES_BLINKING_TIME){
		del_timer_sync(&usb_led_timer);
		led_off(GPIO_USB,-1);
		setup_usb_led_timer();
		usb_turn_on_time = jiffies;
	}
#endif
