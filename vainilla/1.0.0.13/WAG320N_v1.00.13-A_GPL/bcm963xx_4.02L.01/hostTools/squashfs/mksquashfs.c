...
static int devtable_readdir(char *filename, char *dir_name, struct stat* devtable_inode_info);
int delete = FALSE;
...
struct dir_info *dir_scan1(char *, int (_readdir)(char *, char *, struct dir_info *, struct stat * devtable_inode_info), struct stat *devtable_inode_info);
...
int scan1_encomp_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat * devtable_inode_info)
{
	int i, n, pass;
	char *basename;
	static int index = 0;
...
int scan1_single_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat * devtable_inode_info)
{
	struct dirent *d_name;
	int i, pass;
	...
	if((d_name = readdir(dir->linuxdir)) != NULL) {
		...
		return 1;
	}else if(devtable != NULL){
//        if(strncmp(dir->pathname, dev_path, sizeof(dir->pathname)) == 0)
        if(strcmp(dir->pathname, dev_path) == 0)
		{
		    if(devtable_readdir(pathname, dir_name, devtable_inode_info))
		    {
		        return 1;
		    }
		}
...
int scan1_readdir(char *pathname, char *dir_name, struct dir_info *dir, struct stat * devtable_inode_info)
{
	struct dirent *d_name;
	...
	if((d_name = readdir(dir->linuxdir)) != NULL) {
		strcpy(dir_name, d_name->d_name);
		strcat(strcat(strcpy(pathname, dir->pathname), "/"), d_name->d_name);
//		printf("pathname=%s,dir_name=%s\n",pathname,dir_name);
		return 1;
	}else if(devtable != NULL){
//	    printf("scan1_readdir,dir->pathname=%s,dev_path=%s,number=%d\n",dir->pathname,dev_path,sizeof(dir->pathname));
//        if(strncmp(dir->pathname, dev_path, sizeof(dir->pathname)) == 0)
        if(strcmp(dir->pathname, dev_path) == 0)
		{
//		    printf("scan1_readdir,read device table\n");
		    if(devtable_readdir(pathname, dir_name, devtable_inode_info))
		    {
//		        printf("pathname=%s,dir_name=%s\n",pathname,dir_name);
//		        printf("devtable_inode_info->st_mode=%x\n",devtable_inode_info->st_mode);
		return 1;
	}
		}
...
//printf("name=%s,type=%c,mode=%d,uid=%d,gid=%d,major=%d,minor=%d\n",
...
void dir_scan(squashfs_inode *inode, char *pathname, int (_readdir)(char *, char *, struct dir_info *, struct stat * devtable_inode_info))
{
	struct stat buf;
	struct stat devtable_inode_info;
	struct dir_info *dir_info = dir_scan1(pathname, _readdir, &devtable_inode_info);
...
struct dir_info *dir_scan1(char *pathname, int (_readdir)(char *, char *, struct dir_info *, struct stat * devtable_inode_info), struct stat *devtable_inode_info)
{
	...
	while(_readdir(filename, dir_name, dir, devtable_inode_info) != FALSE) {
...
//printf("filename=%s\n",filename);
		...
		if((buf.st_mode & S_IFMT) == S_IFDIR) {
			if((sub_dir = dir_scan1(filename, scan1_readdir,devtable_inode_info)) == NULL)
				continue;
			dir->directory_count ++;
		} else
			sub_dir = NULL;
